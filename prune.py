...
def prune_model(args,
    ...
    """Perform Distilled Pruning (IMP + Distilled Data)
    """
    torch.manual_seed(0)
    ...
    #Save initialization
    torch.save(model.state_dict(), save_path + '_iter0')
    #Create rewind weights
    rewind_model = copy.deepcopy(model).to('cuda:' + str(args.device))

    best_acc, acc = train(args, model, train_loader, val_loader, logger=print)  #pretrain before prune
    for iter in range(max_prune_iters):
        #Prune Model !
        ...
        torch.save(model.state_dict(), save_path + '_iter'+str(iter+1))
...
if __name__ == '__main__':
    ...
    for p in path_list:
        ...
        """
        print('Convnet3 Prune!')
        save_path = '/home/ubuntu/luke/LMC/pruned_models/cifar_10/convnet-3/untrained' + ipc
        prune_model(args, train_loader, val_loader, save_path, model_fn=convnet3, max_prune_iters=10, amount=.2)
        
        print('Resnet10 Prune!')
        save_path = '/home/ubuntu/luke/LMC/pruned_models/cifar_10/resnet-10/untrained' + ipc
        prune_model(args, train_loader, val_loader, save_path, model_fn=resnet10_bn, max_prune_iters=10, amount=.2)
        
        print('Resnet18 Prune!')
        save_path = '/home/ubuntu/luke/LMC/pruned_models/cifar_10/resnet-18/untrained' + ipc
        prune_model(args, train_loader, val_loader, save_path, model_fn=resnet18_bn, max_prune_iters=10, amount=.2)
        """

        
        print('Resnet10 Prune!')
        save_path = '/home/ubuntu/luke/LMC/pruned_models/imagenet_100/resnet-10/untrained' + ipc
        prune_model(args, train_loader, val_loader, save_path, model_fn=resnet10_bn, max_prune_iters=10, amount=.2)
        

        print('Resnet18 Prune!')
        save_path = '/home/ubuntu/luke/LMC/pruned_models/imagenet_100/resnet-18/untrained' + ipc
        prune_model(args, train_loader, val_loader, save_path, model_fn=resnet18_bn, max_prune_iters=10, amount=.2)
