...
def IMP(args, logger):
    ...
    for seed in range(4):
        for prune_iter in range(1,11):
            ...
            if not os.path.exists(path + 'trained_random_prune_iter' + str(prune_iter) + 'seed' + str(seed)):
                np.random.seed(0)
                torch.manual_seed(0)
                torch.cuda.manual_seed(0)
                model = model_fn(args, nclass, logger=logger)
                model.load_state_dict(torch.load(path + 'untrained_iter0'))

                logger(f"\nRepeat: {0}/{10}")
                plotter = Plotter(args.save_dir, args.epochs, idx=0)

                prune_amount = 1-.8**prune_iter
                #Prune Model !  
                prune.global_unstructured(get_parameters_to_prune(model),pruning_method=prune.RandomUnstructured,amount=prune_amount)
                ...
                #Save
                torch.save(model.state_dict(), path + 'trained_random_prune_iter' + str(prune_iter) + 'seed' + str(seed))
