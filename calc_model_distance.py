...
def get_all_distances(args, logger):
    ...
    np.save(path + 'distances_seed08_pruneiter' + str(prune_iter),results)
...
def compute_distance(model_A, model_B):
    ...
    for (name_A, module_A), (name_B, module_B) in zip(model_A.named_modules(), model_B.named_modules()):
        if isinstance(module_A, (torch.nn.Conv2d, torch.nn.Linear, torch.nn.BatchNorm2d, torch.nn.GroupNorm)):
            weights_A = list(module_A.parameters())[0].flatten().detach().cpu().numpy()
            weights_B = list(module_B.parameters())[0].flatten().detach().cpu().numpy()
            
            layer_distance = np.linalg.norm(weights_A - weights_B, ord=2)  # L2 distance
