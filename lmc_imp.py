...
def test_interpolation(args, logger):
    ...
    results = np.load(path + 'train_imp_interpolate_results.npy')
    for prune_iter in range(1,10):
        ...
        seed0_model.load_state_dict(torch.load(path + 'seed0_10_iter' + str(prune_iter) + '_imp'))
        seed1_model.load_state_dict(torch.load(path + 'seed1_10_iter' + str(prune_iter) + '_imp'))
        ...
        for idx, beta in enumerate(np.arange(0,1.1,.1)):
            ...
            results[prune_iter, 0, idx] = loss
            results[prune_iter, 1, idx] = top1
            print('Iter: ', prune_iter, ', Beta: ', beta, ', Loss: ', loss, ', Acc: ', top1)

        np.save(path + 'train_imp_interpolate_results',results)
