...
def test_interpolation(args, logger):
    ...
    if args.model == 'convnet3':
        ...
        if args.dataset == 'cifar10':
            path = '/home/ubuntu/luke/LMC/pruned_models/cifar_10/convnet-3/rewind_'
        ...
        if args.dataset == 'cifar10':
            path = '/home/ubuntu/luke/LMC/pruned_models/cifar_10/resnet-10/rewind_'
        elif args.dataset == 'cifar100':
    ...
    ipc_path = '_' + str(args.ipc_path)
    #ipc_path = ''
    results = np.zeros((11,2,11)) #Prune Iter x (Loss or Acc) x Interpolated Values
    for prune_idx, prune_iter in enumerate([0, 4, 9]): #enumerate(range(11)):
        ...
        seed0_model.load_state_dict(torch.load(path + 'seed0' + ipc_path + '_iter' + str(prune_iter)))
        seed1_model.load_state_dict(torch.load(path + 'seed1' + ipc_path + '_iter' + str(prune_iter)))
        ...
        """
        print('testing a')
        seed0_model = seed0_model.to(torch.device('cuda:' + str(args.device)))
        seed1_model = seed1_model.to(torch.device('cuda:' + str(args.device)))
        top1, top5, loss = validate(args, val_loader, seed0_model, criterion, 0, logger=None)
        print(top1, top5, loss)
        top1, top5, loss = validate(args, val_loader, seed1_model, criterion, 0, logger=None)
        print(top1, top5, loss)3
        print('done')
        """

        zero, total = sparsity_print(seed0_model)
        print('Iter ', prune_iter, ' Sparsity : ', zero / total)
        for idx, beta in enumerate(np.arange(0,1.1,.1)):
            ...
            top1, top5, loss = validate(args, val_loader, interpolated_model, criterion, beta, logger=None) #chagne!
            results[prune_idx, 0, idx] = loss
            results[prune_idx, 1, idx] = top1
            print('Iter: ', prune_iter, ', Beta: ', beta, ', Loss: ', loss, ', Acc: ', top1)

        #np.save(path + 'train_interpolate_results' + ipc_path,results)
