...
def train_subnetworks(args, logger):
    ...
    if args.model == 'convnet3':
        ...
        if args.dataset == 'cifar10':
            ...
            untrained_path = '/home/ubuntu/luke/LMC/pruned_models/imagenet_100/resnet-10/untrained_10_iter'
        ...
        if args.dataset == 'cifar10':
            ...
            untrained_path = '/home/ubuntu/luke/LMC/pruned_models/imagenet_100/resnet-18/untrained_10_iter'
            save_path = '/home/ubuntu/luke/LMC/pruned_models/imagenet_100/resnet-18/'
    else:
        print('No model found')
    arg_seed = args.seed
    for prune_iter in range(10,11):
        for seed in [arg_seed]:
            print('[NEW TRAINING] Prune Iter:', prune_iter, ', seed: ', seed)
            if not os.path.exists(save_path +'seed'+str(seed)+'_10_iter'+str(prune_iter)+'_imp'):
                np.random.seed(seed)
                ...
                model.load_state_dict(torch.load(untrained_path + str(prune_iter) +'_imp'))
                prune.global_unstructured(get_parameters_to_prune(model),pruning_method=prune.L1Unstructured,amount=0)
                best_acc, acc = train(args, model, train_loader, val_loader, plotter, logger)
                torch.save(model.state_dict(), save_path +'seed'+str(seed)+'_10_iter'+str(prune_iter)+'_imp')
